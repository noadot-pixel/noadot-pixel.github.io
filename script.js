document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleFontUpload = (file) => { if (!file) return; const reader = new FileReader(); reader.onload = async (e) => { const fontName = file.name.split('.').slice(0, -1).join('.'); const fontFace = new FontFace(fontName, e.target.result); try { await fontFace.load(); document.fonts.add(fontFace); const option = new Option(fontName, fontName); elements.fontSelect.add(option); option.selected = true; state.textState.fontFamily = fontName; triggerConversion(); } catch (err) { console.error("Font loading failed:", err); alert("폰트 파일을 불러오는 데 실패했습니다."); } }; reader.readAsArrayBuffer(file); };
