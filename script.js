// [v2.4 추가] PNG 메타데이터 처리를 위한 헬퍼 객체
const PNGMetadata = {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // [v2.4 추가] 현재 모든 설정을 하나의 객체로 수집하는 함수
    const gatherSettingsData = () => {
        const settings = {
            version: '2.4', appMode: state.appMode, paletteMode: state.currentMode, useWplaceInGeoMode: state.useWplaceInGeoMode,
            imageControls: { saturation: elements.saturationSlider.value, brightness: elements.brightnessSlider.value, contrast: elements.contrastSlider.value, highQuality: elements.highQualityMode.checked, edgeCleanup: elements.edgeCleanup.checked, ditheringAlgorithm: elements.ditheringAlgorithmSelect.value, ditheringStrength: elements.ditheringSlider.value, },
            scaleControls: { mode: state.scaleMode, width: elements.scaleWidth.value, height: elements.scaleHeight.value, ratio: elements.scaleSlider.value, },
            palettes: {
                geo: Array.from(elements.geoPixelColorsContainer.querySelectorAll('.color-button')).map(b => ({ rgb: b.dataset.rgb, on: b.dataset.on === 'true' })),
                wplaceFree: Array.from(elements.wplaceFreeColorsContainer.querySelectorAll('.color-button')).map(b => ({ rgb: b.dataset.rgb, on: b.dataset.on === 'true' })),
                wplacePaid: Array.from(elements.wplacePaidColorsContainer.querySelectorAll('.color-button')).map(b => ({ rgb: b.dataset.rgb, on: b.dataset.on === 'true' })),
                wplaceFreeInGeo: Array.from(elements.wplaceFreeColorsInGeo.querySelectorAll('.color-button')).map(b => ({ rgb: b.dataset.rgb, on: b.dataset.on === 'true' })),
                wplacePaidInGeo: Array.from(elements.wplacePaidColorsInGeo.querySelectorAll('.color-button')).map(b => ({ rgb: b.dataset.rgb, on: b.dataset.on === 'true' })),
                added: Array.from(elements.addedColorsContainer.querySelectorAll('.added-color-item')).map(item => ({ rgb: JSON.parse(item.dataset.rgb), on: item.dataset.on === 'true' })),
            },
    ...
    // [v2.4 추가] 수집된 설정 객체를 UI에 적용하는 함수
    const applySettingsData = (settings) => {
        if (!settings || (settings.version !== '2.3' && settings.version !== '2.4')) { alert('호환되지 않거나 설정 정보가 없는 이미지입니다.'); return; }
        setAppMode(settings.appMode); setPaletteMode(settings.paletteMode);
        elements.useWplaceInGeoMode.checked = settings.useWplaceInGeoMode; elements.wplacePaletteInGeo.style.display = settings.useWplaceInGeoMode ? 'block' : 'none';
        const ic = settings.imageControls; elements.saturationSlider.value = ic.saturation; elements.saturationValue.textContent = ic.saturation; elements.brightnessSlider.value = ic.brightness; elements.brightnessValue.textContent = ic.brightness; elements.contrastSlider.value = ic.contrast; elements.contrastValue.textContent = ic.contrast; elements.highQualityMode.checked = ic.highQuality; elements.edgeCleanup.checked = ic.edgeCleanup; elements.ditheringAlgorithmSelect.value = ic.ditheringAlgorithm; elements.ditheringSlider.value = ic.ditheringStrength; elements.ditheringValue.textContent = ic.ditheringStrength; state.highQualityMode = ic.highQuality; state.edgeCleanup = ic.edgeCleanup;
        const sc = settings.scaleControls; state.scaleMode = sc.mode; elements.scaleModeSelect.value = sc.mode; elements.scaleWidth.value = sc.width; elements.scaleHeight.value = sc.height; elements.scaleSlider.value = sc.ratio; elements.scaleValue.textContent = (sc.ratio / CONFIG.SCALE_FACTOR).toFixed(2); updateScaleUIVisibility();
        const applyPaletteState = (container, savedState) => { savedState.forEach(s => { const btn = container.querySelector(`.color-button[data-rgb='${s.rgb}']`); if (btn && btn.dataset.on !== String(s.on)) { btn.click(); } }); };
        applyPaletteState(elements.geoPixelColorsContainer, settings.palettes.geo); applyPaletteState(elements.wplaceFreeColorsContainer, settings.palettes.wplaceFree); applyPaletteState(elements.wplacePaidColorsContainer, settings.palettes.wplacePaid); applyPaletteState(elements.wplaceFreeColorsInGeo, settings.palettes.wplaceFreeInGeo); applyPaletteState(elements.wplacePaidColorsInGeo, settings.palettes.wplacePaidInGeo);
        elements.addedColorsContainer.innerHTML = ''; settings.palettes.added.forEach(c => createAddedColorItem(c, c.on));
        state.textState = settings.textControls; elements.editorTextarea.value = state.textState.content; elements.fontSelect.value = state.textState.fontFamily; elements.fontSizeSlider.value = state.textState.fontSize; elements.fontSizeValue.textContent = state.textState.fontSize; elements.letterSpacingSlider.value = state.textState.letterSpacing; elements.letterSpacingValue.textContent = state.textState.letterSpacing; elements.paddingSlider.value = state.textState.padding; elements.paddingValue.textContent = state.textState.padding; elements.textColorSelect.value = state.textState.textColor; elements.bgColorSelect.value = state.textState.bgColor; elements.strokeColorSelect.value = state.textState.strokeColor; elements.strokeWidthSlider.value = state.textState.strokeWidth; elements.strokeWidthValue.textContent = state.textState.strokeWidth;
    ...
    // [v2.4 수정] handleFile 함수 전체 교체
    const handleFile = async (file) => {
        if (!file || !file.type.startsWith('image/')) return;
        try { const settings = await PNGMetadata.extract(file); if (settings) { if (confirm("메타데이터가 기록되어있습니다. 저장된 설정을 불러오시겠습니까?")) { applySettingsData(settings); } } } catch (error) { console.error("메타데이터 읽기 오류:", error); }
        state.originalFileName = file.name; const img = new Image(); img.onload = () => {
    ...
    const setupEventListeners = () => {
        ...
        elements.downloadBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                const baseName = state.originalFileName.substring(0, state.originalFileName.lastIndexOf('.')) || state.originalFileName; const newName = `${baseName}_NoaDot_${ts}_settings.png`;
