...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const elements = {
        ...
        highQualityMode: document.getElementById('highQualityMode'), // <-- Element 추가
    ...
    const applyConversion = async (imageDataToProcess, activePalette, options) => {
        ...
        // 최적화: 고품질 모드일 경우, 팔레트의 Oklab 값을 미리 한 번만 계산
        ...
        if (activePalette.length === 0) {
            ...
            for (let y = 0; y < height; y++) {
                ...
                for (let x = 0; x < width; x++) {
                    ...
                    // 팔레트의 RGB값과 미리 계산된 Oklab 값을 함께 전달
    ...
    const resetAddedColors = () => {
        if (!elements.addedColorsContainer.querySelector('.added-color-item')) {
            alert('초기화할 색상이 없습니다.');
            return;
        }

        if (confirm('정말로 추가한 모든 색상을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.')) {
    ...
    const setupEventListeners = () => {
        ...
        // ▼▼▼▼▼ 추가: 이벤트 리스너 ▼▼▼▼▼
        elements.highQualityMode.addEventListener('change', (e) => {
            state.highQualityMode = e.target.checked;
            triggerConversion();
        });
        // ▲▲▲▲▲ 추가된 이벤트 리스너 종료 ▲▲▲▲▲
