document.addEventListener('DOMContentLoaded', () => {
    ...
    const CONFIG = {
        ...
        DEFAULTS: { 
            ...
            ditheringSlider: 0, scaleSlider: 4
        }
    };
    ...
    const elements = {
        loadingIndicator: document.getElementById('loading-indicator'), // ## 핵심 변경 ##
    ...
    const showLoading = (visible) => {
        elements.loadingIndicator.classList.toggle('visible', visible);
    };
    ...
    const populateColorSelects = () => {
        ...
        if (state.currentMode === 'geopixels') {
            palettes.push({ groupName: 'GeoPixels', data: geoPixelHexCodes.map(hex => ({ rgb: hexToRgb(hex), name: hex })) });
        } else {
            palettes.push({ groupName: 'Wplace 무료', data: wplaceFreeColorsData });
            palettes.push({ groupName: 'Wplace 유료', data: wplacePaidColorsData });
        }
    ...
    const setupEventListeners = () => {
        ...
        ['saturation', 'brightness', 'contrast', 'dithering', 'scale'].forEach(t => { const s = elements[`${t}Slider`], v = elements[`${t}Value`]; if(s && v) { s.addEventListener('input', () => { if (t === 'scale') { v.textContent = (s.value / CONFIG.SCALE_FACTOR).toFixed(2); } else { v.textContent = s.value; } triggerConversion(); }); } });
        document.querySelectorAll('.reset-btn').forEach(btn => { btn.addEventListener('click', () => { const targetId = btn.dataset.target; const slider = elements[targetId]; const valueDisplay = elements[`${targetId.replace('Slider', '')}Value`]; const defaultValue = CONFIG.DEFAULTS[targetId]; slider.value = defaultValue; if (targetId === 'scaleSlider') { valueDisplay.textContent = (defaultValue / CONFIG.SCALE_FACTOR).toFixed(2); } else { valueDisplay.textContent = defaultValue; } triggerConversion(); }); });
