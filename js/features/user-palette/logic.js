...
export class UserPaletteFeature {
    ...
    // [New] 모드 전환 시 통계 및 추천 초기화
    resetStats() {
        this.lastPixelStats = {};
        // 추천 목록도 비우기 위해 빈 배열로 렌더링
    ...
    initEvents() {
        if (this.ui.addBtn) {
            this.ui.addBtn.addEventListener('click', () => this.handleManualAdd());
        }

        if (this.ui.resetBtn) {
            this.ui.resetBtn.addEventListener('click', () => {
                if (state.addedColors.length === 0) {
                    alert(t('alert_no_color_to_reset'));
                    return;
                }
        ...
        if (this.ui.exportBtn) {
            this.ui.exportBtn.addEventListener('click', () => {
                if (state.addedColors.length === 0) {
                    alert(t('alert_no_color_to_export'));
                    return;
                }
                ...
                a.href = url;
                a.download = "noadot_palette.json";
                ...
                reader.onload = (e) => {
                    ...
                    if (newColors && newColors.length > 0) {
                        ...
                        newColors.forEach(rgb => {
                            if (!this.algo.isDuplicate(rgb, state.addedColors)) {
                                state.addedColors.push(rgb);
                                addedCount++;
                            }
                        });
                        if (addedCount > 0) {
                            alert(t('alert_palette_imported', { n: addedCount }));
                            this.updateStateAndUI();
                        } else {
                            alert(t('alert_no_addcolor'));
                        }
                    } else {
                        alert(t('alert_long_filecode'));
                    }
    ...
    handleManualAdd() {
        ...
        if (this.algo.validateInput('rgb', r) && 
            this.algo.validateInput('rgb', g) && 
            this.algo.validateInput('rgb', b)) {
            this.addColor([r, g, b]);
            this.ui.clearInputs();
        } else {
            if (hexInputValue.length > 0) {
                alert(t('alert_invalid_color_code'));
            } else {
                alert(t('alert_input_color_value'));
            }
        }
    }
